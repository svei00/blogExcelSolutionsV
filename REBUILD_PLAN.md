...
| 11.A.5 Response cache ✅ **DONE (2026-08-28)** + nginx `location = /`/`= /search` with the 502 fallback ⏳ **CODE READY, NGINX STEP MANUAL/PENDING** | New `api/lib/responseCache.js` - module-level `Map`, 60s TTL, no invalidation hooks into `post.controller.js` on purpose (zero coupling worth more than instant freshness here). Wired into all three `injectMeta.js` handlers (`post:<slug>`, `home`, `archive` cache keys); 404s and the filtered-`/search` fallthrough are deliberately never cached (cheap/not-this-handler's-job respectively). **Verified live against the real server, not just reasoned about:** repeated requests confirmed byte-identical (`diff` on the response body) and dramatically faster on the cache hit - `/search` 257ms → 2ms, `/post/:slug` 50ms → 1.4ms. Regression-checked filtered `/search`, an unknown slug, and `/about` - none of them cache or behave differently. ⚠️ **nginx side is still a manual VPS step, not done** - exact snippet handed to svei (mirrors the existing `/post/` proxy pattern + the 502→static-file fallback from 11.0). **Non-negotiable deploy order, same as every prior nginx change in this project (notes.md 27.1): push → deploy → `curl` the live Express process directly to confirm `/` and `/search` respond correctly → only then reload nginx.** Reloading nginx first would 502 the entire homepage the instant the location block takes effect. |
