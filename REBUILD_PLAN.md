...
**Feasibility: confirmed, spike run 2026-07-19.** `@toast-ui/editor` is MIT, markdown-native (paste Markdown → it *stays* Markdown), dual WYSIWYG/markdown modes, dark theme included, Spanish i18n included. No blocker found — Toast UI stays the pick. **Three real findings from the spike, none fatal, all actionable (folded into 2.4 below):**
- **`@toast-ui/react-editor` is dead — do not install it.** Its peer dependency has been pinned to `react@^17.0.1` since its last publish (2023-02-17, v3.2.3) and was never updated for React 18. `npm install` throws `ERESOLVE` under this project's React 18.3.1. **Fix:** use `@toast-ui/editor` (the core, framework-agnostic package — zero peer deps) directly, driven by our own `useRef`/`useEffect` wrapper. This installed cleanly with no conflicts and is the standard integration pattern for Toast UI under React 18/19 anyway — one fewer abandoned dependency, not a compromise.
- **Image insertion mid-paragraph, then continued typing, works cleanly** — verified live in both WYSIWYG and markdown modes: cursor lands correctly after the inserted image, typing continues normally, no crash, no stuck dialog. Markdown round-trips correctly (`![alt](url)` inline, alt text preserved). Confirms 6.10 can stay 100% render-time as planned.
- **No live theme-switching API.** Checked the installed package's own type definitions (`node_modules/@toast-ui/editor/types/editor.d.ts`) — `theme` is a constructor-only option; there is no `setTheme()` method on the `Editor` class. Toggling dark mode requires destroying and re-instantiating the editor, not a live call. This project already has a dark-mode toggle (`ThemeProvider.jsx`) — 2.4 needs to handle re-mount-on-theme-change explicitly, not assume a live switch.

*Flagged alternative:* **Tiptap** (MIT core) — much more active, but markdown in/out requires extra extensions and wiring; more build effort for your use case. Not needed — no blocker surfaced.
...
| Task | Hint |
|------|------|
| **2.1 Spike** ✅ **DONE 2026-07-19, branch `spike-toastui-2.1` (throwaway, not merged)** | Findings above the table. Summary: use `@toast-ui/editor` core directly (not the dead `@toast-ui/react-editor` wrapper), mid-paragraph image insertion + continued typing confirmed clean in both modes, dark-mode toggle needs destroy/re-create not a live API. No blocker — proceed with 2.2 onward. |
...
| **2.4 Editor component** | `client/src/components/PostEditor.jsx` wraps **`@toast-ui/editor` (core package, not `@toast-ui/react-editor`** — see 2.1 findings: the React wrapper is unpublished since 2023 and ERESOLVEs against React 18) via `useRef`/`useEffect`, matching the spike's pattern: construct on mount, `.destroy()` on unmount. Props in (`value`, `onChange`), markdown out via `getMarkdown()`. **Dark mode:** since `theme` is constructor-only (2.1 finding — no live `setTheme()`), the component must fully re-mount (destroy + reconstruct) when the app's dark-mode flag flips, not attempt a live theme swap. Image upload moves to its own hook `client/src/hooks/useImageUpload.js` (Firebase logic extracted from `CustomReactQuill.jsx` + `CreatePost.jsx` — one copy instead of two). Toast UI's `addImageBlobHook` connects the two. |
...
|---|---|---|
| @toast-ui/editor (core only — `@toast-ui/react-editor` dropped, see 2.1) | MIT | Phase 2 |
| marked | MIT | Phase 2 |
