...
**More bugs found on the live post page (`/post/xlookup-vs-indexmatch...`), one fixed, one flagged:**
...
**Still open before Phase 7 can close:** a real Lighthouse pass once a Chrome binary is available in this environment (axe-core covers most of the same rule set but isn't a substitute for the actual score), and the alt-text decision above.

**Done when:** Lighthouse a11y ≥ 95 on Home and post page; zero serious axe violations; all text tokens documented ≥ 4.5:1. Axe/manual-audit criterion met on every page audited including the post page; Lighthouse score remains outstanding; alt-text on legacy posts is a known, flagged gap pending svei's call on approach.
