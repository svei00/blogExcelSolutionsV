...
export default function DashUsers() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const res = await authFetch(`/api/user/getusers`);
  ...
  const handleShowMore = async () => {
    ...
    try {
      const res = await authFetch(`/api/user/getusers?startIndex=${startIndex}`);
      const data = await res.json();
      if (res.ok) {
  ...
  const handleDeleteUser = async () => {
    setShowModal(false);
    try {
      const res = await authFetch(`/api/user/delete/${userIdToDelete}`, {
        method: "DELETE",
      });
      const data = await res.json();
